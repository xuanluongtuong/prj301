...
(
	ID INT PRIMARY KEY IDENTITY(1,1) NOT NULL,
	IDCV INT,
	FOREIGN KEY (IDCV) REFERENCES dbo.CONGVIEC(ID) ON DELETE CASCADE,	
...
INNER JOIN dbo.NHANVIEN ON dbo.ACCOUNT.accountID = dbo.NHANVIEN.MANV
INNER JOIN dbo.PHONGBAN ON dbo.NHANVIEN.MAPB = dbo.PHONGBAN.MAPB
WHERE email = N'thaiph@gmail.com'

SELECT CV.ID,CV.IDPC, PC.MADA,TENDA, PC.MAPB, HO_VA_TEN, CV.MANV, TEN_CONG_VIEC,CV.NGAYBATDAU, CV.NGAYKETTHUC, CV.TRANGTHAI, CV.PHEDUYET
...
WHERE CV.MANV=9 ORDER BY CV.NGAYBATDAU DESC